...
let handler = async (m, { conn, text, usedPrefix, participants, groupMetadata }) => {
    ...
    if (res.status !== 200) throw `Maaf *${joiner}* Bot Tidak Dapat Masuk,, mungkin link telah direset ulang atau Bot telah dikeluarkan oleh seseorang.`
    if (global.DATABASE.data.chats[m.chat].welcome) return false
