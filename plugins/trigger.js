...
try {
  await m.reply('Sedang membuat...')
    ...
    let ger = isQuotedImage ? JSON.parse(JSON.stringify(m).replace('quotedM','m')).message.extendedTextMessage.contextInfo : m
        ...
        let img = await imageToBase64(`http://zekais-api.herokuapp.com/trigger?url=${teks}`)
          let buffer = Buffer.from(img, 'base64')
          let hasil = await sticker(buffer, false, 'Triggered', 'Kuhong Bot')

      conn.sendFile(m.chat, hasil, 'trigger.webp', '', m, false, { asSticker: true })

    await m.reply('Nihh')
        } else {
    m.reply(`Kirim perintah ${usedPrefix}trigger dengan caption digambar atau reply gambar yang tersedia`)
     }
 } catch (e) {
