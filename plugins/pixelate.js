...
`.trim()
 ...
 try {
  await m.reply('Sedang membuat...')
	...
	let buff = `http://zekais-api.herokuapp.com/pixelate?url=${teks}`
          let voss = await fetch(buff)
	  let vuss = await ftype.fromStream(voss.body)
          if (vuss !== undefined) {

	 conn.sendFile(m.chat, await getBuffer(buff), '8bit.jpg', caption,  m)

     }	} else {
    m.reply(`Kirim perintah ${usedPrefix}8bit dengan caption digambar atau reply gambar yang tersedia`)
     }
 } catch (e) {
