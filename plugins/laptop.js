...
`.trim()
 ...
 try {
  await m.reply('Sedang membuat...')
	...
	let buff = `https://videfikri.com/api/textmaker/customwp/?urlgbr=${teks}`
          let voss = await fetch(buff)
	  let vuss = await ftype.fromStream(voss.body)
          if (vuss !== undefined) {

	 conn.sendFile(m.chat, await getBuffer(buff), 'laptop.jpg', caption,  m)

     }	} else {
    m.reply(`Kirim perintah ${usedPrefix}laptop dengan caption digambar atau reply gambar yang tersedia`)
     }
 } catch (e) {
