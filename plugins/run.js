let handler = async (m, { conn, text, args, usedPrefix }) => {
 ...
 try {
  ...
  if (!m.quoted) return conn.reply(m.chat, 'Tag Fotonya!', m)
  if ((isMedia && m.message.imageMessage || isQuotedImage || !isQuotedImage) && args.length == 0) {
  m.reply('Sedang membuat...\nMohon tunggu sekitar 1 menit')
  running(await q).then(vid => conn.sendFile(m.chat, vid, 'run.mp4', 'Nih Video Bergeraknya,,\n\nSemoga suka yaa', m))
   } else throw `Kirim perintah ${usedPrefix}run dicaption media gambar atau reply media gambar yang tersedia`
 } catch (e) {
